...
async def start(bot, message):
    await bot.send_message(chat_id=update.chat.id, text=Message.START_TEXT.format(update.from_user.mention), parse_mode="html", disable_web_page_preview=True, reply_markup=START_BUTTON, reply_to_message_id=update.message_id)

@Client.on_message(filters.command(["help"]))
async def help(bot, message):
    await bot.send_message(chat_id=update.chat.id, text=Message.HELP_USER, parse_mode="html", disable_web_page_preview=True, reply_markup=HELP_BUTTON, reply_to_message_id=update.message_id)
